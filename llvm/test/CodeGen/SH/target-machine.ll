...
; RUN: llc -mtriple=sh-unknown-elf -mcpu=sh2 -relocation-model=pic -filetype=null < %s
; RUN: not llc -mtriple=sh-unknown-elf -mcpu=sh2 -relocation-model=dynamic-no-pic < %s -o /dev/null 2>&1 | FileCheck %s --check-prefix=RELOCATION
; RUN: not llc -mtriple=sh-unknown-elf -mcpu=sh2 -relocation-model=ropi < %s -o /dev/null 2>&1 | FileCheck %s --check-prefix=RELOCATION
; RUN: not llc -mtriple=sh-unknown-elf -mcpu=sh2 -relocation-model=rwpi < %s -o /dev/null 2>&1 | FileCheck %s --check-prefix=RELOCATION
; RUN: not llc -mtriple=sh-unknown-elf -mcpu=sh2 -relocation-model=ropi-rwpi < %s -o /dev/null 2>&1 | FileCheck %s --check-prefix=RELOCATION
...
; RELOCATION: LLVM ERROR: SH only supports static and position-independent relocation models
